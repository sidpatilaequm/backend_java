...
public class FinancialTermsService {
    private static final Logger logger = LoggerFactory.getLogger(FinancialTermsService.class);
    ...
    @Transactional
    public ServiceResponse saveFinancialTerms(FinancialTermsDTO dto) {
        ...
        try {
            ...
            if (!"vendor".equalsIgnoreCase(dto.getAuthKey())) {
                return serviceControllerUtils.prepareMobileResponseErrorStatus(
                    response,
                    AppConstants.ERRORCODE,
                    "Financial terms can only be added for vendors"
                );
            }
            ...
            List<CompanyDetails> companies = companyDetailsRepository.findByGstinNumberAndSuperAdmin_SuperAdminIdAndAuthKey(
                dto.getGstinNumber(), superAdmin.getSuperAdminId(), "vendor");
            ...
            List<FinancialTerms> existingTerms = financialTermsRepository.findByCompany_CompanyId(company.getCompanyId());
            ...
            financialTerms = financialTermsRepository.save(financialTerms);
    ...
    public ServiceResponse getFinancialTermsByGstAndAuthKey(String gstinNumber, String authKey) {
        ServiceResponse response = new ServiceResponse();
        ...
        try {
            ...
            // Validate auth key
            if (!"vendor".equalsIgnoreCase(authKey)) {
                return serviceControllerUtils.prepareMobileResponseErrorStatus(
                    response,
                    AppConstants.ERRORCODE,
                    "Financial terms can only be retrieved for vendors"
                );
            }

            // Find company by GST number for current super admin
            List<CompanyDetails> companies = companyDetailsRepository.findByGstinNumberAndSuperAdmin_SuperAdminIdAndAuthKey(
                gstinNumber, superAdmin.getSuperAdminId(), "vendor");
            if (companies.isEmpty()) {
                return serviceControllerUtils.prepareMobileResponseErrorStatus(
                    response,
                    AppConstants.ERRORCODE,
            ...
            List<FinancialTerms> financialTerms = financialTermsRepository.findByCompany_CompanyId(company.getCompanyId());
    ...
    @Transactional
    public ServiceResponse updateFinancialTerms(FinancialTermsDTO dto) {
        ServiceResponse response = new ServiceResponse();
        ...
        try {
            ...
            if (!"vendor".equalsIgnoreCase(dto.getAuthKey())) {
                return serviceControllerUtils.prepareMobileResponseErrorStatus(
                    response,
                    AppConstants.ERRORCODE,
                    "Financial terms can only be updated for vendors"
                );
            }
            ...
            FinancialTerms financialTerms = existingTerms.get(0);
            financialTerms.setDeliveryTerms(dto.getDeliveryTerms());
            financialTerms.setDeliveryLocation(dto.getDeliveryLocation());
            financialTerms.setBlockIndicator(dto.getBlockIndicator());
            financialTerms.setOrderCurrency(dto.getOrderCurrency());
